...
def load_ff_csv_generic(csv_path: str) -> pd.DataFrame:
    ...
    def _finish(df):
        ...
        def _maybe_scale(s):
            ...
            return s/100.0 if (med is not None and med > 0.01) else s
        for c in cols:
        ...
        if not isinstance(df.index, pd.DatetimeIndex):
            ...
            if idx.isna().all():
                # Try day-first dates already on the index (e.g., 31/07/1963)
...
def load_ff5_csv(csv_path: str) -> pd.DataFrame:
    """
    Load Ken French 5-Factor *monthly* CSV robustly (handles new 2025 format).

    Approach:
      • Read raw text (to skip the prose preamble).
      • Find the header line that contains ',Mkt-RF,SMB,HML,RMW,CMA,RF'.
      • Collect subsequent lines that start with 'YYYYMM,'.
      • Parse to DataFrame; convert YYYYMM to month-end dates.
      • Rename Mkt-RF→MKT and scale % → decimals.
    """
    from pathlib import Path
    import io, re
    ...
    # 1) Locate the factor header (it literally starts with a comma in the download)
    try:
        hdr_idx = next(
            i for i, l in enumerate(lines)
            if re.match(r'^\s*,\s*Mkt-RF\s*,\s*SMB\s*,\s*HML\s*,\s*RMW\s*,\s*CMA\s*,\s*RF\s*$', l.strip())
        )
    except StopIteration:
        raise ValueError(
            "Couldn't find the ',Mkt-RF,SMB,HML,RMW,CMA,RF' header. "
            "Are you sure this is the monthly 5-factor CSV?"
        )

    # 2) Collect contiguous YYYYMM rows under the header
    start = hdr_idx + 1
    block = []
    for l in lines[start:]:
        if re.match(r'^\s*\d{6}\s*,', l):   # a monthly row
            block.append(l.strip())
        else:
            break

    if not block:
        raise ValueError("Found the header but no YYYYMM rows underneath. The file may be the wrong variant (e.g., annual/daily).")

    # 3) Build a minimal CSV string and load it
    header = lines[hdr_idx].lstrip(",").strip()       # drop the leading comma
    mini_csv = header + "\n" + "\n".join(block)

    df = pd.read_csv(io.StringIO(mini_csv), index_col=0)
    # Index: YYYYMM -> month-end DatetimeIndex
    df.index = pd.to_datetime(df.index.astype(str), format="%Y%m") + pd.offsets.MonthEnd(0)

    # 4) Canonicalise names and scale to decimals
...
def pick_ff_csv_gui() -> str | None:
    ...
    try:
        ...
        path = filedialog.askopenfilename(
            parent=root, title="Select Fama–French Factors CSV (5 or 5+Momentum)",
...
def run_regression_once(ff_csv_path: str, start_month: pd.Period, end_month: pd.Period,
    ...
    params = model.params.rename({'const': 'Alpha'}) * 0.01  # convert from % to decimals
    ...
    params.plot(kind="bar", color=colors, ax=ax,
            title=f"{fund_name} — Alpha & Factor Exposures{title_suffix}")
    ax.axhline(0, linewidth=0.8)
    ax.set_ylabel("Coefficient (decimal)")
    ax.grid(False)
...
def main():
    ...
    print("\nOpening file dialog to select your Fama–French Factors CSV (5 or 5+Momentum)…")
